    ...
    <div class="mt-10 glossy-card flex flex-col w-full max-w-4xl mx-4 sm:mx-auto rounded-lg text-left shadow-xl transform transition-all relative z-[10000] max-h-[80vh]">
      ...
      <div class="flex-1 overflow-y-auto p-4">
        <div class="grid grid-cols-5 gap-4">
          <!-- Work order details - Takes up 3 columns -->
          <div class="col-span-3 text-gray-300 space-y-4">
            <!-- Work order details -->
            ...
            <div class="flex justify-between items-start glossy-section">
              <div class="w-full">
                ...
                <div class="flex items-center">
                  <span v-if="!editingField.customer_id" class="text-white">
                    {{ workOrder.customer?.business_name || 'No Customer' }}
                  </span>
                  <select v-else v-model="form.customer_id" class="mt-1 block w-full rounded-md bg-gray-900 border-gray-800 shadow-sm focus:border-lime-600 focus:ring-lime-600 text-white sm:text-sm">
              ...
              <div>
                <p class="text-sm text-lime-400">Hourly Rate:</p>
                <div class="flex items-center">
            ...
            <div class="flex justify-between items-start glossy-section">
              <div class="w-full">
                <p class="text-sm text-lime-400">Total:</p>
                <div class="flex items-center">
                  <span class="text-white">
                    {{ workOrder.customer?.pay_rate && workOrder.hours ? `$${(parseFloat(workOrder.customer.pay_rate) * parseFloat(workOrder.hours)).toFixed(2)}` : 'N/A' }}
                  </span>
                </div>
              </div>
            </div>
            ...
            <!-- User section -->
            <div>
              <p class="text-sm text-lime-400">Assigned Technician:</p>
              <div class="flex items-center">
                <span v-if="!editingField.user_id" class="text-white">
                  {{ workOrder.technician ? workOrder.technician.name : 'No technician assigned' }}
                </span>
                <select 
                  v-else 
                ...
                >
                  ...
                  <option v-for="tech in $page.props.technicians" :key="tech.id" :value="tech.id">
                    {{ tech.name }}
                  </option>
                </select>
                <span class="ml-2">
                  ...
                  <button v-else @click="saveField('technician_id')" class="text-green-400 btn hover:text-green-300">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                  </button>
          ...
          <!-- Timeline section - Takes up 2 columns -->
          <div class="col-span-2 sticky top-0">
            <Timeline :workOrderId="workOrder.id" />
          </div>

          <!-- Go back to main column for notes -->
...
<script>
  ...
  import PdfThumbnail from '@/Components/PdfThumbnail.vue';  import NetworkStatusIndicator from '@/Components/NetworkStatusIndicator.vue';
    ...
    setup(props, { emit }) {
      ...
      watch(() => props.workOrder, (newVal) => {
        if (newVal) {
          ...
          // Log to help debug customer data
          console.log('Work Order loaded with customer:', newVal.customer);
      ...
      const saveField = async (field) => {
        ...
        // Special handling for date fields
          ...
          // Handle customer_id field specifically
          data.customer_id = form.customer_id;
          console.log('Updating customer_id to:', form.customer_id);
        } else {
        ...
        try {
          ...
          if (response.data.success) {
            ...
            if (field === 'hours') {
              ...
              // Handle customer ID update - reload the work order to get full customer data
              if (response.data.workOrder && response.data.workOrder.customer) {
                props.workOrder.customer = response.data.workOrder.customer;
                props.workOrder.customer_id = data.customer_id;
              } else {
                // Fallback: just update the ID and set business_name to pending
                props.workOrder.customer_id = data.customer_id;
                props.workOrder.customer = {
                  id: data.customer_id,
                  business_name: 'Loading...'
                };
                
                // Optionally reload the full work order
                axios.get(`/work-orders/${props.workOrder.id}/details`)
                  .then(detailsResponse => {
                    if (detailsResponse.data && detailsResponse.data.customer) {
                      props.workOrder.customer = detailsResponse.data.customer;
                    }
                  })
                  .catch(error => console.error('Failed to reload customer details:', error));
              }
            } else {
      ...
      // Define getStatusProgress function
