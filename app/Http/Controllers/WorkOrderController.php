...
{
    try {
        // Find the work order and load activities with their associated users
        $workOrder = WorkOrder::findOrFail($id);
        $activities = $workOrder->activities()->with('user:id,name,email')->orderBy('created_at', 'desc')->get();
