// Script to improve URL handling for PdfThumbnail
const fs = require('fs');
const path = require('path');

// Path to files
const workOrderPath = path.join(__dirname, 'resources', 'js', 'Pages', 'WorkOrders', 'WorkOrder.vue');

// Read the file content
let workOrderContent = fs.readFileSync(workOrderPath, 'utf8');

// Define the improved getAttachmentUrl function
const improvedGetAttachmentUrl = `// Helper: Get the full URL for an attachment
...
// Define the old getAttachmentUrl function pattern using regex
const oldGetAttachmentUrlRegex = /\/\/ Helper: Get the full URL for an attachment\s*function getAttachmentUrl\(attachment\)\s*{[\s\S]*?return\s*'';?\s*}/;

// Replace the old function with the improved one
const updatedWorkOrderContent = workOrderContent.replace(oldGetAttachmentUrlRegex, improvedGetAttachmentUrl);

// Save the updated file
if (workOrderContent !== updatedWorkOrderContent) {
  fs.writeFileSync(workOrderPath, updatedWorkOrderContent, 'utf8');
  console.log('Successfully updated getAttachmentUrl function in WorkOrder.vue');
} else {
  console.log('No changes were made to WorkOrder.vue');
}

console.log('URL handling fixes completed!');
