<template>
  <div>
    <button @click="toggleWeekends"
      class="mb-4 px-4 py-2 bg-indigo-600 dark:bg-lime-400 text-black rounded-md hover:bg-indigo-500 dark:hover:bg-lime-500 transition">
      Toggle Weekends
    </button>
      ...
      <FullCalendar :options="calendarOptions">
        ...
        <template v-slot:eventContent="arg">
          <div class="w-full flex rounded-md shadow-sm event-card overflow-hidden">
            ...
            <div :style="{ backgroundColor: getStatusColor(arg.event.extendedProps.status) }"
              class="flex w-8 shrink-0 items-center justify-center rounded-l-md text-xs font-medium text-black border-r border-white/20 px-2">
              {{ arg.event.extendedProps.status.charAt(0).toUpperCase() }}
            </div>

            <!-- Right content section -->
            <div
              class="flex flex-1 items-center justify-between truncate rounded-r-md bg-white/10 dark:bg-gray-700/40 backdrop-blur-sm">
              <div class="flex-1 p-4 truncate px-2 py-1 text-xs">
                <p class="font-medium text-white text-xs dark:gray-900 truncate">{{ arg.event.title }}</p>
...
/* Style the calendar headers */
.fc .fc-col-header-cell {
  background-color: #0b1b45; /* Dark blue background */
}
